...
class CanvasView(QGraphicsView):
    ...
    def mouseMoveEvent(self, e):
        ...
        if self.resizing_brush:
            ...
            if self.adjusting_brush_strength:
                delta_x = (
                    e.position().x()
                    - self.brush_strength_start_x
                )

                delta_y = (
                    self.brush_strength_start_y
                    - e.position().y()
                )
            ...
            if delta_x != 0:
                ...
                new_size = max(
                    1,
                    int(current_size + delta_x * scale),
                )
    ...
    def paintEvent(self, e):
        ...
        if (
            self.resizing_brush
            and self.adjusting_brush_strength
        ):
    ...
    def keyPressEvent(self, e):
        ...
        if (
            ...
            if mime.hasImage():
                ...
                if not image.isNull():
                    self.window.add_image_from_clipboard(
                        image
                    )
    ...
    def get_brush_mask(self):
        ...
        # Нелинейное преобразование hardness.
        hardness_curve = (
            hardness_value
            * hardness_value
            * (3.0 - 2.0 * hardness_value)
    ...
    def paint_line_to(self, pos):
        ...
        # Если предыдущей точки нет, просто ставим один отпечаток.
        if (
            self.last_stroke_x is None
            or self.last_stroke_y is None
        ):
            self.paint_at(pos)
        ...
        step = max(
            1.0,
            self.window.brush_size * 0.20,
        )

        steps = max(
            1,
            int(np.ceil(distance / step)),
        )
    ...
    def paint_at(self, pos):
        ...
        if self.brush_shift_locked:
            if (
                self.last_paint_x is not None
                and self.last_paint_y is not None
            ):
        ...
        if self.brush_shift_locked:
            ...
            blur_radius = max(
                1,
                int(np.ceil((smooth_radius / 2.0) * 3.0)),
            )
